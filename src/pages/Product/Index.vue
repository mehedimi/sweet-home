...
function expand(index) {
  if (expandIndex.value === index) {
    ...
    useApi
      .get(`/products/${products.value[index].ID}/stocks`)
      .then(({ data }) => {
...
function incrementOrDecrementStockQuantity(index, mode) {
  useApi
    .patch(`/stocks/${stockItems.value[index].ID}/quantity/${mode}`)
    .then(() => {
      switch (mode) {
        case "increment":
          stockItems.value[index].comment_count =
            typeof stockItems.value[index].comment_count === "string"
              ? parseInt(stockItems.value[index].comment_count) + 1
              : stockItems.value[index].comment_count + 1;
          break;
        case "decrement":
          stockItems.value[index].comment_count =
            typeof stockItems.value[index].comment_count === "string"
            ...
            <tbody class="bg-white dark:bg-slate-800">
              <template v-for="(product, index) in products">
                <tr>
                  ...
                  <td
                    class="border-b border-slate-200 dark:border-slate-600 p-4 pr-8 text-slate-500 dark:text-slate-400 text-right"
                  >
                    <button
                      @click="expand(index)"
                      class="bg-indigo-500 text-white py-1 px-3 rounded text-xs"
                      v-text="expandIndex === index ? 'Collapse' : 'Expand'"
                    ></button>
                    <button
                      @click="deleteProduct(index)"
                      class="bg-red-500 text-white py-1 px-3 rounded text-xs ml-3"
                    >
                      Delete
                    </button>
                  </td>
                ...
                <tr v-if="index === expandIndex">
                  <td colspan="4">
                    <table class="w-full">
                      ...
                      <tbody>
                        <tr
                          v-for="(item, stockIndex) in stockItems"
                        ...
                        >
                          ...
                          <td>
                            <button
                              @click="
                                incrementOrDecrementStockQuantity(
                                  stockIndex,
                                  'decrement'
                                )
                              "
                              class="bg-sky-500 text-white py-1 px-3 rounded text-xs mr-3"
                            >
                              -
                            </button>
                            {{ item.comment_count }}
                            <button
                              ...
                              "
                              class="bg-emerald-500 text-white py-1 px-3 rounded text-xs ml-3"
                            >
                              +
                            </button>
                          </td>
                          <td>
                            <button
                              @click="deleteStockItem(stockIndex)"
                              class="bg-red-500 text-white py-1 px-3 rounded text-xs ml-3"
                            >
                              Delete
                            </button>
